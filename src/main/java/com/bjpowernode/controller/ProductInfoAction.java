...
//    }
    ...
    public String save(ProductInfo info,HttpServletRequest request){
        ...
        //info对象中右边但提交的5个数据，有异步ajax上来的图片名称数据，上架时间数据
        int num = -1;
        num = productInfoService.save(info);
        if(num>0)
            request.setAttribute("msg","增加成功");
        else
            request.setAttribute("mas","增加失败");
        //清空saveFileName变量中的内容，为了下次增加或修改的异步ajax的上传处理
        saveFileName="";
        //增加成功后应从新访问数据哭，跳转到分页显示
    ...
    public String update(ProductInfo info,HttpServletRequest request){
        //因为ajax的异步图片上传，如果有上传过，则saveFikeName里有上传上来的图片名称
        //如果没有使用异步ajax上传图片，则saveFileName="";实体类Info使用隐藏表单提供上来的pImage原始图片名称
        ...
        //处理完更新后，saveFileName里有可能有数据，应该清空，为下次更新作依据
        saveFileName="";
        return "forward:/prod/split.action";
    }
    ...
    public String deleteBatch(String pids,HttpServletRequest request){
        ...
        try {
            ...
            request.setAttribute("msg","该商品不能够删除");
        }
        return "forward:/prod/deleteAjaxSplit.action";
    }
